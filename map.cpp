...
namespace EScript
{
    ...
    void Map::init(EScript::Namespace& globals)
    {
        ...
        ES_CONSTRUCTOR(typeObject, 0, -1,
                       {
                           if((parameter.count() % 2) == 1)
                               rt.warn("Map: Last parameter ignored!");
                           Map* a = new Map;
        ...
        ES_MFUN(typeObject, Map, "unset", 1, 1, (thisObj->unset(parameter[0]), thisEObj))

        //! [ESMF] bool Collection.containsKey(Object)
        ES_MFUN(typeObject, Map, "containsKey", 1, 1, thisObj->getValue(parameter[0].toString()) != nullptr)
        ...
        ES_MFUN(typeObject, Map, "swap", 1, 1, (thisObj->swap(assertType<Map>(rt, parameter[0])), thisEObj))
    }
