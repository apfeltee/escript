...
namespace EScript
{
    ...
    void Array::init(EScript::Namespace& globals)
    {
        ...
        ES_MFUN(typeObject, Array, "append", 1, 1, (thisObj->append(parameter[0].to<Collection*>(rt)), thisEObj))
        ...
        ES_MFUN(typeObject, Array, "removeValue", 1, 3,
                (thisObj->rt_removeValue(rt, parameter[0], parameter[1].toInt(-1), parameter[2].toUInt(0)), thisEObj))
        ...
        ES_MFUN(typeObject, Array, "splice", 2, 3,
                (thisObj->splice(parameter[0].to<int>(rt), parameter[1].to<int>(rt),
                                 parameter.count() > 2 ? assertType<Array>(rt, parameter[2]) : nullptr),
                 thisEObj))

        //! [ESMF] Array Array.slice( start,length );
        ES_MFUN(typeObject, const Array, "slice", 1, 2,
                thisObj->slice(parameter[0].to<int>(rt), parameter[1].toInt(0)).detachAndDecrease())
