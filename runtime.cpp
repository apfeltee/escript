...
namespace EScript
{
    ...
    void Runtime::init(EScript::Namespace& globals)
    {
        ...
        ES_FUN(typeObject, "_setStackSizeLimit", 1, 1, (rt._setStackSizeLimit(parameter[0].to<uint32_t>(rt)), RtValue(nullptr)))
        ...
        ES_FUN(typeObject, "setTreatWarningsAsError", 1, 1, (rt.setTreatWarningsAsError(parameter[0].toBool()), RtValue(nullptr)))
    ...
    //! (ctor)
    Runtime::Runtime()
    : ExtObject(Runtime::getTypeObject()), internals(new RuntimeInternals(*this)), logger(new LoggerGroup(Logger::LOG_WARNING))
    {
    ...
    bool Runtime::checkNormalState() const
    {
        return internals->checkNormalState();
    }
    ...
    ObjRef Runtime::createInstance(const EPtr<Type>& type, const ParameterValues& _params)
    {
        ...
        ObjRef resultObj;
        if(callResult.isFunctionCallContext())
        {// user function?
    ...
    ObjRef Runtime::executeFunction(const ObjPtr& fun, const ObjPtr& caller, const ParameterValues& _params)
    {
        ...
        if(internals->getState() == RuntimeInternals::STATE_EXCEPTION)
        {// error occurred? throw an exception! callResult is undefined.
            throw internals->fetchAndClearException().detachAndDecrease();
        }
        return resultObj;
    }

    ObjRef Runtime::fetchAndClearExitResult()
    {
        return internals->fetchAndClearExitResult();
    }

    ObjPtr Runtime::getCallingObject() const
    {
        return internals->getCallingObject();
    }

    std::string Runtime::getCurrentFile() const
    {
        return internals->getCurrentFile();
    }

    int Runtime::getCurrentLine() const
    {
        return internals->getCurrentLine();
    }

    Namespace* Runtime::getGlobals() const
    {
        return internals->getGlobals();
    }

    std::string Runtime::getStackInfo()
    {
        return internals->getStackInfo();
    }

    size_t Runtime::getStackSize() const
    {
        return internals->getStackSize();
    }

    size_t Runtime::_getStackSizeLimit() const
    {
    ...
    void Runtime::setAddStackInfoToExceptions(bool b)
    {
        internals->setAddStackInfoToExceptions(b);
    }

    void Runtime::_setExceptionState(ObjRef e)
    {
        internals->setExceptionState(std::move(e));
    }

    void Runtime::setException(const std::string& s)
    {
        internals->setException(s);
    }

    void Runtime::setException(Exception* e)
    {
        internals->setException(e);
    }

    void Runtime::_setExitState(ObjRef e)
    {
        internals->setExitState(e);
    }

    void Runtime::_setStackSizeLimit(const size_t s)
    {
        internals->_setStackSizeLimit(s);
    }

    void Runtime::setTreatWarningsAsError(bool b)
    {
        if(b)
        {// --> disable coutLogger and add throwLogger
            ...
            {
                Runtime& rt;
                void doLog(level_t, const std::string& message) override
                {
                    rt.setException(message);
                }
    ...
    {
        internals->throwException(s, obj);
    }
    ...
    class CountingLogger : public Logger
    {
        ...
        void doLog(level_t l, const std::string&) override
        {
            ++counter[l];
        }
