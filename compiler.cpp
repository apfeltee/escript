...
namespace EScript
{
    ...
    void Compiler::addExpression(FnCompileContext& ctxt, EPtr<AST::ASTNode> expression) const
    {
        ...
        {// make sure that something is added to the stack
    ...
    void Compiler::addStatement(FnCompileContext& ctxt, EPtr<AST::ASTNode> statement) const
    {
        ...
        {// if something is added to the stack, remove it.
    ...
    void Compiler::finalizeInstructions(InstructionBlock& instructionBlock)
    {
        ...
        {// pass 2: adapt jump instructions
            ...
            {
                if(instruction.getType() == Instruction::I_JMP || instruction.getType() == Instruction::I_JMP_IF_SET
                   || instruction.getType() == Instruction::I_JMP_ON_TRUE || instruction.getType() == Instruction::I_JMP_ON_FALSE
    ...
    {                                                                     \
        ...
        {                                                                 \
            ...
            {                                                             \
                ...
                if(!self)                                                 \
                    throw std::invalid_argument("Compiler: Wrong type!"); \
                do                                                        \
                    _block while(false);                                  \
            }                                                             \
        };                                                                \
        ...
        // Void
        ADD_HANDLER(ASTNode::TYPE_VALUE_VOID, AST::VoidValueExpr, { ctxt.addInstruction(Instruction::createPushVoid()); })
        ...
        ADD_HANDLER(ASTNode::TYPE_BLOCK_EXPRESSION, Block,
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if(c + 1 == self->getStatements().end())
                                {// last statemenet ? --> keep the result
        ...
        ADD_HANDLER(ASTNode::TYPE_EXIT_STATEMENT, ExitStatement,
                    {
                        ...
                        ctxt.addInstruction(Instruction::createSysCall(Consts::SYS_CALL_EXIT, self->getValueExpression() ? 1 : 0));
                    })
        // FunctionCallExpr
        ADD_HANDLER(
        ASTNode::TYPE_FUNCTION_CALL_EXPRESSION, FunctionCallExpr,
        {
            ...
            if(!self->isSysCall())
            {
                do
                {
                    ...
                    if(gAttr)
                    {
                        ...
                        if(gAttr->getObjectExpression() == nullptr)
                        {// singleIdentifier (...)
                            const auto varLocation = ctxt.getCurrentVarLocation(attrId);
                            if(isValidVarLocation(varLocation))
                            {// localOrStaticVar(...)
                                if(!self->isConstructorCall())
                                {// constructor calls don't need a caller
                                    ctxt.addInstruction(Instruction::createPushVoid());
                                }
                                if(isLocalVarLocation(varLocation))
                                {
            ...
            if(self->hasExpandingParameters())
            {
                ...
                ctxt.addInstruction(Instruction::createSysCall(
                Consts::SYS_CALL_EXPAND_PARAMS_ON_STACK, static_cast<uint32_t>(self->getExpandingParameters().size() + 1)));
        ...
        ADD_HANDLER(ASTNode::TYPE_LOOP_STATEMENT, LoopStatement,
                    {
                        ...
                        if(self->getPostConditionExpression())
                        {// increaseStmt is ignored!
        ...
        ADD_HANDLER(ASTNode::TYPE_SWITCH_STATEMENT, SwitchCaseStatement,
                    {
                        ...
                        while(!caseMarkers.empty())
                        {// add remaining case markers (after the last instruction)
                            ctxt.addInstruction(Instruction::createSetMarker(caseMarkers.front().second));
                            caseMarkers.pop_front();
                        }
        ...
        ADD_HANDLER(ASTNode::TYPE_THROW_STATEMENT, ThrowStatement,
                    {
                        if(self->getValueExpression())
                            ctxt.addExpression(self->getValueExpression());
                        ctxt.addInstruction(Instruction::createSysCall(Consts::SYS_CALL_THROW, self->getValueExpression() ? 1 : 0));
                    })

        // TryCatchStatement
        ADD_HANDLER(
        ASTNode::TYPE_TRY_CATCH_STATEMENT, TryCatchStatement,
        {
        ...
        // user function
        ADD_HANDLER(
        ASTNode::TYPE_USER_FUNCTION_EXPRESSION, UserFunctionExpr,
        {
            ERef<UserFunction> fun = new UserFunction;
            ...
            for(const auto& param : self->getParamList())
            {
                ...
                if(param.isMultiParam())
                {
                    ...
                    ctxt2.addInstruction(Instruction::createSysCall(Consts::SYS_CALL_TEST_ARRAY_PARAMETER_CONSTRAINTS,
                                                                    typeExpressions.size() + 1));
                ...
                else
                {
                    ...
                    ctxt2.addInstruction(Instruction::createGetLocalVariable(varIdx));
                    ctxt2.addInstruction(
                    Instruction::createSysCall(Consts::SYS_CALL_THROW_TYPE_EXCEPTION, constrainOkMarkers.size() + 1));
                    ...
                    for(std::vector<uint32_t>::const_reverse_iterator cIt = constrainOkMarkers.rbegin();
                        cIt != constrainOkMarkers.rend(); ++cIt)
                    {
