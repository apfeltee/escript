 ...
 */
    static annotations_t getAnnotations(ParsingContext& ctxt, int from, int to)
    {
        ...
        for(int cursor = from; cursor <= to; ++cursor)
        {
            ...
            int optionPos = -1;
            ++cursor;
            if(Token::isA<TStartBracket>(tokens.at(cursor)) && cursor < to)
            {// annotation has options 'annotation(exp1,exp2)'
                optionPos = cursor;
            ...
            if(cursor <= to && !Token::isA<TDelimiter>(tokens.at(cursor)))
            {// expect a delimiter or the end.
    ...
    static EPtr<AST::ASTNode>
    createAssignmentExpr(ParsingContext& ctxt, const LValueInfo& lValue, AST::ASTNode* rightExpression, int currentLine, int cursor);
 ...
 */
    void pass_1(ParsingContext& ctxt)
    {
        ...
        struct _BracketInfo
        {
            Token* token;
            unsigned int index;
            bool isBlockOrMap, containsColon, containsCommands, nextCBlockIsSwitchCaseBlock, isSwitchCaseBlock;
            int shortIf;// a?b:c

            _BracketInfo(unsigned int _index = 0, Token* _token = nullptr)
            : token(_token), index(_index), isBlockOrMap(false), containsColon(false), containsCommands(false),
              nextCBlockIsSwitchCaseBlock(false), isSwitchCaseBlock(false), shortIf(0){};
 ...
 */
    void pass_2(ParsingContext& ctxt, Tokenizer::tokenList_t& enrichedTokens)
    {
        ...
        for(size_t cursor = 0; cursor < ctxt.tokens.size(); ++cursor)
        {
            ...
            switch(token->getType())
            {
                case TControl::TYPE_ID:
                {
                    ...
                    /// for(...) ---> for{...}
                    else if(tc->getId() == Consts::IDENTIFIER_for || tc->getId() == Consts::IDENTIFIER_foreach
                            || tc->getId() == Consts::IDENTIFIER_while)
                    {
 ...
 */
    EPtr<AST::ASTNode> readExpression(ParsingContext& ctxt, int& cursor, int to)
    {
        ...
        /// "(a+2)"
        /// --------------------------
        else if(Token::isA<TStartBracket>(tokens.at(cursor)) && Token::isA<TEndBracket>(tokens[to])
        ...
        /// "{foo:bar,2:3}"
        /// --------------------------
        if(Token::isA<TStartMap>(tokens.at(cursor)) && Token::isA<TEndMap>(tokens[to])
 ...
 */
    ...
    createAssignmentExpr(ParsingContext& ctxt, const LValueInfo& lValue, AST::ASTNode* rightExpression, int currentLine, int cursor)
    {
        ...
        else if(lValue.type == LValueInfo::COLLECTION_SETTER)
        {
            ...
            return FunctionCallExpr::createFunctionCall(new GetAttributeExpr(lValue.objExpression, Consts::IDENTIFIER_fn_set),
	...
	*/
        else if(lValue.type == LValueInfo::ARRAY_OF_LVALUES)
        {
            ...
            for(const auto& subLValue : lValue.lValueArray)
            {
                ...
                //			paramExp.push_back();
                block->addStatement(createAssignmentExpr(
                ctxt, subLValue,
                FunctionCallExpr::createFunctionCall(
                new GetAttributeExpr(new GetAttributeExpr(nullptr, VAR_TMP), Consts::IDENTIFIER_fn_get), paramExp, currentLine),
                currentLine, cursor));
            }
    ...
    EPtr<AST::ASTNode> readBinaryExpression(ParsingContext& ctxt, int& cursor, int to)
    {
        ...
        for(int i = cursor; i <= to; ++i)
        {
            ...
            if(level == 0)
            {
                ...
                if(top && (top->getAssociativity() == Operator::L ? top->getPrecedence() >= opPrecedence : top->getPrecedence() > opPrecedence))
                {
            ...
            if(Token::isA<TStartBlock>(t) || Token::isA<TStartBracket>(t) || Token::isA<TStartIndex>(t) || Token::isA<TStartMap>(t))
            {
                ++level;
                continue;
            }
            else if(Token::isA<TEndBlock>(t) || Token::isA<TEndBracket>(t) || Token::isA<TEndIndex>(t) || Token::isA<TEndMap>(t))
            {
                --level;
                if(level < 0)
                {
        ...
        else if(op->getString() == ":=" || op->getString() == "::=")
        {
                ...
                if(annotationStart > 0 && atOp != nullptr && atOp->toString() == "@")
                {
                    ...
                    for(const auto& annotation : annotations)
                    {
                        ...
                        else if(name == Consts::ANNOTATION_ATTR_member)
                        {
                            if(flags & Attribute::TYPE_ATTR_BIT)
                            {
                                log(ctxt, Logger::LOG_WARNING,
                                    "'@(member)' is used in combination with @(type) or '::=' and is ignored.", atOp);
                            }
                            else
                            {
                        ...
                        else if(name == Consts::ANNOTATION_ATTR_private)
                        {
                            ...
                            {
                                log(ctxt, Logger::LOG_WARNING,
                                    "'@(private)' is used in combination with @(public) and is ignored.", atOp);
                            }
                            else
                            {
                        ...
                        else if(name == Consts::ANNOTATION_ATTR_public)
                        {
                            ...
                            {
                                log(ctxt, Logger::LOG_WARNING,
                                    "'@(public)' is used in combination with @(private) and is ignored.", atOp);
                            }
                            else
                            {
            ...
            {
                log(ctxt, Logger::LOG_WARNING,
                    "Use '=' for assigning to local variables instead of '" + op->getString() + "' ", tokens[opPosition]);
                return SetAttributeExpr::createAssignment(lValue.objExpression, lValue.variableName, rightExpression, currentLine);
            }
        ...
        if(leftExpression.isNull())
        {
            ...
            new GetAttributeExpr(rightExpression, std::string(op->getString()) + "_pre"), ASTNode::refArray_t(), currentLine);
            return fc;
        }
        ...
        else if(rightExpression.isNull())
        {
            ...
            FunctionCallExpr* fc = FunctionCallExpr::createFunctionCall(
            new GetAttributeExpr(leftExpression, std::string(op->getString()) + "_post"), ASTNode::refArray_t(), currentLine);
            cursor--;
        ...
        else
        {
            ASTNode::refArray_t paramExp;
            paramExp.push_back(rightExpression);
            return FunctionCallExpr::createFunctionCall(new GetAttributeExpr(leftExpression, op->getString()), paramExp, currentLine);
        }
    }
 ...
 */
	...
	*/
		...
		*/
            ...
            EPtr<AST::ASTNode> loopInit = SetAttributeExpr::createAssignment(
            nullptr, itId,
            FunctionCallExpr::createSysCall(Consts::SYS_CALL_GET_ITERATOR, loopInitParams, tokens.at(cursor)->getLine()));

            // ! __it.end()
            EPtr<AST::ASTNode> checkExpression = LogicOpExpr::createNot(FunctionCallExpr::createFunctionCall(
            new GetAttributeExpr(new GetAttributeExpr(nullptr, itId), Consts::IDENTIFIER_fn_it_end), ASTNode::refArray_t()));

            // __it.next()
            EPtr<AST::ASTNode> increaseStatement = FunctionCallExpr::createFunctionCall(
            ...
            if(keyIdent)
            {
                actionWrapper->addStatement(SetAttributeExpr::createAssignment(
                nullptr, keyIdent->getId(),
                FunctionCallExpr::createFunctionCall(
                new GetAttributeExpr(new GetAttributeExpr(nullptr, itId), Consts::IDENTIFIER_fn_it_key), ASTNode::refArray_t()),
            ...
            {
                actionWrapper->addStatement(SetAttributeExpr::createAssignment(
                nullptr, valueIdent->getId(),
                FunctionCallExpr::createFunctionCall(
                new GetAttributeExpr(new GetAttributeExpr(nullptr, itId), Consts::IDENTIFIER_fn_it_value), ASTNode::refArray_t()),
                tokens.at(cursor)->getLine()));
            }
            actionWrapper->addStatement(action);

            loopWrappingBlock->addStatement(
            LoopStatement::createForLoop(loopInit, checkExpression, increaseStatement, actionWrapper, elseAction));
...
*/
    ...
    //!	(ctor)
    Parser::Parser(Logger* _logger) : logger(_logger ? _logger : new StdLogger(std::cout))
    {
        //ctor
    }
