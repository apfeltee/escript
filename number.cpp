...
namespace EScript
{
    ...
    void Number::init(EScript::Namespace& globals)
    {
        ...
        ES_MFUN(typeObject, Number, "format", 0, 4,
                thisObj->format(static_cast<std::streamsize>(parameter[0].to<int>(rt, 3)), parameter[1].toBool(true),
        ...
        ES_FUN(typeObject, "log", 0, 1,
               (parameter.count() > 0 ? log(thisEObj->toDouble()) / log(parameter[0].to<double>(rt)) : log10(thisEObj->toDouble())))
