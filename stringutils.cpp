...
namespace EScript
{
    double StringUtils::readNumber(const char* s, std::size_t& cursor, bool checkSign)
    {
        ...
        else if(c == '0' && (s[cursor + 1] == 'b' || s[cursor + 1] == 'B'))
        {// binaryNumber
            ++cursor;
            double number = 0;
            while(true)
    ...
    std::string
    StringUtils::replaceMultiple(const std::string& subject, const std::vector<std::pair<std::string, std::string>>& rules, int max)
    {
 ...
 */
    std::string StringUtils::UCS2LE_to_ANSII(const std::string& source)
    {
        ...
        for(size_t i = 0; i < length; i += 2)
        {
            ...
            {
                s << "&#x" << std::hex << std::setfill('0') << static_cast<int>(0x0100 * c[i + 1] + c[i]) << std::dec << ";";
            }
        }
        return s.str();
    }
        ...
        if(len > 0)
        {
            ...
            else
            {
                ...
                for(int i = 1; i != max && cursor <= len - delimiterLen; ++i)
                {
                    ...
                    if(cursor == len)
                    {// ending on delimiter? -> add empty part
                        result.push_back("");
                    }
                }
                if(cursor < len)
