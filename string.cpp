 ...
 */
    ...
    void String::init(EScript::Namespace& globals)
    {
        ...
        ES_MFUN(typeObject, const String, "beginsWith", 1, 2,
                thisObj->sData.beginsWith(parameter[0].toString(), parameter[1].to<uint32_t>(rt, 0)))

        //! [ESMF] Bool String.contains (String)search [,(Number)startIndex] )
        ES_MFUN(typeObject, const String, "contains", 1, 2,
                thisObj->getString().find(parameter[0].toString(), thisObj->sData.codePointToBytePos(parameter[1].toUInt(0)))
                != std::string::npos)
        ...
        ES_MFUNCTION(typeObject, const String, "endsWith", 1, 1,
                     {
                         ...
                         return thisObj->getString().size() >= search.size()
                                && thisObj->getString().compare(thisObj->getString().length() - search.length(),
        ...
        ES_MFUNCTION(typeObject, const String, "replaceAll", 1, 3,
                     {
                         ...
                         else
                         {
                             ...
                             return StringUtils::replaceAll(subject, parameter[0].toString(), parameter[1].toString(),
                                                            parameter[2].toInt(-1));
                         }
                     })
