...
namespace EScript
{
    ...
    Token* Tokenizer::readNextToken(const std::string& codeU8, std::size_t& cursor, int& line, size_t& startPos, tokenList_t& tokens)
    {
        ...
        else if(isChar(c))
        {
            ...
            else if(id == Consts::IDENTIFIER_LINE)
            {// __LINE__
        ...
        else if(isOperator(c))
        {
            ...
            if(op->getString() == "-")
            {
                ...
                if(last == nullptr
                   || (!(Token::isA<TEndBracket>(last) || Token::isA<TEndIndex>(last) || Token::isA<TIdentifier>(last)
                         || Token::isA<TValueBool>(last) || Token::isA<TValueIdentifier>(last) || Token::isA<TValueString>(last)
                         || Token::isA<TValueNumber>(last) || Token::isA<TValueVoid>(last))))
                {
                    // TODO ++,--
