...
namespace EScript
{
    ...
    {
        struct factory
        {// use factory for static one time initialization
            Type* operator()()
            {
    ...
    void Type::init(EScript::Namespace& globals)
    {
        ...
        ES_CONSTRUCTOR(typeObject, 0, 1,
                       {
                           Type* baseType
                           = parameter.count() == 0 ? ExtObject::getTypeObject() : assertType<Type>(rt, parameter[0]);
    ...
    //! (ctor)
    Type::Type() : Object(Type::getTypeObject()), flags(0), baseType(Object::getTypeObject())
    {
        //ctor
    }

    //! (ctor)
    Type::Type(Type* _baseType) : Object(Type::getTypeObject()), flags(0), baseType(_baseType)
    {
    ...
    //! (ctor)
    Type::Type(Type* _baseType, Type* typeOfType) : Object(typeOfType), flags(0), baseType(_baseType)
    {
        if(getBaseType() != nullptr)
            getBaseType()->copyObjAttributesTo(this);
        //ctor
