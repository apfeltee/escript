...
namespace EScript
{
    ...
    ObjRef RuntimeInternals::executeFunctionCallContext(_Ptr<FunctionCallContext> fcc)
    {
        ...
        while(true)
        {
            ...
            if(fcc->getInstructionCursor() == instructions.end())
            {
                ...
                fcc = getActiveFCC();
                if(fcc.isNull())
                {//! just to be safe (should never occur)
                    return result;
                }
			...
			In other words: Use 'continue' only if no exception or warning may occur.*/
                            ...
                            if(attr->getProperties() & Attribute::ASSIGNMENT_RELEVANT_BITS)
                            {
                                if(attr->isConst())
                                {
                                    setException("Cannot assign to const attribute '"
                                                 + instruction.getValue_Identifier().toString() + "'.");
                                    break;
                                }
                                else if(attr->isPrivate() && fcc->getCaller() != obj)
                                {
                                    setException("Cannot access private attribute '" + instruction.getValue_Identifier().toString()
                                                 + "' from outside of its owning object.");
                                    break;
                                }
                    ...
                    case Instruction::I_INIT_CALLER:
                    {
                        ...
                        if(fcc->isConstructorCall())
                        {
                            ...
                            if(result.isFunctionCallContext())
                            {
                                ...
                                fcc = result._getFCC();
                                pushActiveFCC(fcc);
                                for(std::vector<ObjPtr>::const_reverse_iterator it = constructors.rbegin();
                                    it != constructors.rend(); ++it)
				...
				obj.identifier @(properties) := value	*/
                        ...
                        if((properties & Attribute::OVERRIDE_BIT)
                           && (obj->_accessAttribute(instruction.getValue_Identifier(), false) == nullptr))
                        {
                        ...
                        {
                            warn("Setting type attribute '" + instruction.getValue_Identifier().toString()
                                 + "' to an object which is no Type.");
                        }
				...
				push result (or jump to exception point)	*/
                        ...
                        = (v.second == Consts::DYNAMIC_PARAMETER_COUNT) ? fcc->stack_popUInt32() : v.second;
        ...
        switch(fun->_getInternalTypeId())
        {
            case _TypeIds::TYPE_USER_FUNCTION:
            {
                ...
                {// multiParameter
                    ...
                    {
                        const auto firstMultiParamValue
                        = std::min(pValues.end(), std::next(pValues.begin(), userFunction->getMultiParam()));
                    ...
                    if(fcc->getLocalVariableName(variableIdx).empty())
                    {// empty parameter name? -> ignore the values
                        ++variableIdx;
                        valueIt
                        = std::next(pValues.begin(), pValues.size() + 1 + multiParamIndex - userFunction->getParamCount());
                        for(; valueIt < pValues.end(); ++valueIt, ++variableIdx)// assign the remaining values
                            fcc->assignToLocalVariable(variableIdx, *valueIt);
                    }
                    else if(valueIt >= pValues.end())
                    {// multi parameter lies behind the actually given parameters: fn(a=1,m...){} ()
                        fcc->assignToLocalVariable(Consts::LOCAL_VAR_INDEX_firstParameter + multiParamIndex, Array::create());// assign an empty array
                    }
                    else
                    {// copy values into multiParam
                        EPtr<Array> multiParamArray = Array::create();
                        ObjRef arrayRef(multiParamArray.get());
                        const auto pValuesEnd
                        = std::next(pValues.begin(), pValues.size() + 1 + multiParamIndex - userFunction->getParamCount());
            ...
            case _TypeIds::TYPE_FN_BINDER:
            {
                ...
                if(binder->getBoundParameters().empty())
                {
                    return startFunctionExecution(binder->getFunction(),
                                                  binder->getObject() ? binder->getObject() : _callingObject, pValues);
                }
                else
                {
                    ...
                    for(auto& param : pValues)
                        pValues2.set(i++, param);
                    return startFunctionExecution(binder->getFunction(),
                                                  binder->getObject() ? binder->getObject() : _callingObject, pValues2);
                }
            }
            case _TypeIds::TYPE_FUNCTION:
            {// is  C++ function ?
    ...
    //! (internal)
    RtValue RuntimeInternals::startInstanceCreation(EPtr<Type> type, ParameterValues& pValues)
    {// add caller as parameter?
        std::vector<ObjPtr> constructors;
        ...
        if(!constructors.empty())
        {
            ...
            if(result.isFunctionCallContext())
            {
                ...
                for(std::vector<ObjPtr>::const_reverse_iterator it = constructors.rbegin();
                    std::next(it) != constructors.rend(); ++it)
                    fcc->stack_pushObject(*it);
    ...
    std::string RuntimeInternals::getStackInfo()
    {
        ...
        for(std::vector<_CountedRef<FunctionCallContext> >::reverse_iterator it = activeFCCs.rbegin();
            it != activeFCCs.rend(); ++it)
        {
            ++nr;
            ...
            else
            {
                ...
                os << "\n\n" << nr << '.' << "\t(" << activeFun->getCode().getFilename() << ":" << activeLine << ')';

                if(activeLine >= 0)
                {
                    os << "\nCode:\t'"
                       << StringUtils::trim(StringUtils::getLine(activeFun->getCode().getFullCode(), activeLine - 1)) << "'";
                }
...
#define ES_SYS_FUNCTION(_name) \
    static EScript::RtValue _name(EScript::RuntimeInternals& rtIt UNUSED_ATTRIBUTE, const EScript::ParameterValues& parameter UNUSED_ATTRIBUTE)
    ...
    {                                                                           \
        ...
        {//! [ESSF] Map SYS_CALL_CREATE_MAP( key0,value0, key1,value1, ... )
            struct _
            {
                ...
                {
                    if((parameter.count() % 2) == 1)
                        rtIt.warn("Map: Last parameter ignored!");
                    Map* a = Map::create();
        ...
        {//! [ESSF] Iterator SYS_CALL_GET_ITERATOR( object );
            struct _
            {
                ES_SYS_FUNCTION(sysCall)
                {
                    ...
                    {
                        it = std::move(callMemberFunction(rtIt.runtime, parameter[0], Consts::IDENTIFIER_fn_getIterator,
                                                          ParameterValues()));
                    }
                    if(it == nullptr)
                    {
            ...
            struct _
            {
                ES_SYS_FUNCTION(sysCall)
                {
                    ...
                    for(const auto& val : *values)
                    {
                        ...
                        {
                            ObjRef result = std::move(callMemberFunction(
                            rtIt.runtime, parameter[i], Consts::IDENTIFIER_fn_checkConstraint, ParameterValues(val.get())));
                            if(result.toBool())
                            {
                                success = true;
                                break;
		...
		*/
            struct _
            {
                ES_SYS_FUNCTION(sysCall)
                {
                    ...
                    if(attr.isNull())
                    {// first call -> set attribute and don't skip statement
                        fcc->getUserFunction()->setAttribute(markerId, create(nullptr));// store void
                        return false;
                    }
                    else
                    {// already called -> skip statement
                        return true;
                    }
                }
            };
