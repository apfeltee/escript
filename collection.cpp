...
namespace EScript
{
    ...
    void Collection::init(EScript::Namespace& globals)
    {
        ...
        ES_MFUN(typeObject, Collection, "_set", 2, 2, (thisObj->setValue(parameter[0], parameter[1]), thisEObj))
        ...
        ES_MFUN(typeObject, Collection, "set", 2, 2, (thisObj->setValue(parameter[0], parameter[1]), thisEObj))
        ...
        ES_MFUN(typeObject, Collection, "max", 0, 0, thisObj->rt_extract(rt, Consts::IDENTIFIER_fn_greater, true))

        //! [ESMF] Object Collection.min()
        ES_MFUN(typeObject, Collection, "min", 0, 0, thisObj->rt_extract(rt, Consts::IDENTIFIER_fn_greater, false))

        //! [ESMF] bool Collection.contains(Object)
        ES_MFUN(typeObject, Collection, "contains", 1, 1, thisObj->rt_contains(rt, parameter[0]))

        //! [ESMF] KEY Collection.findValue( VALUE )
        ES_MFUN(typeObject, Collection, "findValue", 1, 1, thisObj->rt_findValue(rt, parameter[0]))
